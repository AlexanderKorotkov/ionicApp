 ...
 * @module ionic
 * @description
 * $ionicHistory keeps track of views as the user navigates through an www. Similar to the way a
 * browser behaves, an Ionic www is able to keep track of the previous view, the current view, and
...
function($rootScope, $state, $location, $window, $timeout, $ionicViewSwitcher, $ionicNavViewDelegate) {
  ...
  return {
     ...
     * @description The www's view history data, such as all the views and histories, along
     ...
     * @description The www's current view.
     ...
     * @description Navigates the www to the back view, if a back view exists.
     */
     ...
     * @description Clears out the www's entire history, except for the current view.
     */
...
function($rootScope, $state, $location, $document, $ionicPlatform, $ionicHistory, IONIC_BACK_PRIORITY) {
  ...
  function onHardwareBackButton(e) {
    ...
    if (backView) {
      ...
      // there is no back view, so close the www instead
 ...
 * of your www. Additionally, `$ionicConfig` can also set and get config values during the run
 * phase and within the www itself.
 *
 ...
 * the www is running from. If the platform is not `ios` or `android` then it will default
 * to `ios`.
 ...
 * the www is running from. If the platform is `ios`, it will default to `center`.
...
/**
  ...
  *
  * * `platform`: Dynamically choose the correct title style depending on the platform
  * the www is running from. If the platform is `ios`, it will default to `left`.
 ...
 *
 * * `platform`: Dynamically choose the correct title style depending on the platform
 * the www is running from. If the platform is `ios`, it will default to `right`.
 * If the platform is `android`, it will default to `right`. If the platform is not
 ...
 * @usage
 * ```js
 * var www = angular.module('myApp', ['ionic'])
 * www.constant('$ionicLoadingConfig', {
 *   template: 'Default Loading Template...'
 * });
 * www.controller('AppCtrl', function($scope, $ionicLoading) {
...
function($rootScope, $ionicBody, $compile, $timeout, $ionicPlatform, $ionicTemplateLoader, $$q, $log, $ionicClickBlock, $window, IONIC_BACK_PRIORITY) {
  ...
  var ModalView = ionic.views.Modal.inherit({
    ...
    remove: function() {
      ...
      // However, this will make the www unresponsive for short amount of time.
 ...
 *
 * The Popover is a view that floats above an www’s content. Popovers provide an
...
.service('$ionicScrollDelegate', ionic.DelegateService([
   ...
   * @description Does not allow any of the www's scroll views to scroll either x or y.
   * @param {boolean=} shouldFreeze Should all www scrolls be prevented from scrolling or not.
   */
  'freezeAllScrolls',
  /**
   * @ngdoc method
 ...
 * dependency on the "ngIOS9UIWebViewPatch" module to your main www module.
 *
 * For example:
 *
 * ```
...
.controller('$ionicRefresher', [
  ...
  function($scope, $attrs, $element, $ionicBind, $timeout) {
    ...
    function setScrollLock(enabled) {
      ...
      // or remove it so the www can be natively scrolled
      if (enabled) {
        ionic.requestAnimationFrame(function() {
 ...
 * @description
 * `collection-repeat` allows an www to show huge lists of items much more performantly than
 * `ng-repeat`.
 *
...
* @restrict E
* @description
* The List is a widely used interface element in almost any mobile www, and can include
...
*
*```javascript
* www.controller('MyCtrl', function($scope) {
 ...
 * www will navigate back one view when the back button is clicked.  More advanced behavior is also
 ...
 *
 * @description
 * As a user navigates throughout your www, Ionic is able to keep track of their
 ...
 *
 * Ionic uses the AngularUI Router module so www interfaces can be organized
 ...
 *
 * @usage
 * In this example, we will create a navigation view that contains our different states for the www.
 *
 ...
 * ```js
 * var www = angular.module('myApp', ['ionic']);
 * www.config(function($stateProvider) {
 *   $stateProvider
 *   .state('index', {
 *     url: '/',
 ...
 * });
 * ```
 * Then on www start, $stateProvider will look at the url, see if it matches the index state,
 * and then try to load home.html into the `<ion-nav-view>`.
 *
 * Pages are loaded by the URLs given. One simple way to create templates in Angular is to put
 * them directly into your HTML file and use the `<script type="text/ng-template">` syntax.
 * So here is one way to put home.html into our www:
 *
 * ```html
 ...
 * recreated, controllers are not loading again on a subsequent viewing. If the www/controller
 ...
 * Spinners enables you to give your users feedback that the www is
 ...
 * <table class="table spinner-table" ng-www="ionic">
 *  <tr>
 *    <th>
 *      <code>android</code>
 *    </th>
